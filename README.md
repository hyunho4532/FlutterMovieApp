...
- ## 2023-08-23
   - [X] 다이얼로그에서 영화 입력 클릭 시 새로운 다이얼로그 창 구현 완료
   - [X] 새로운 다이얼로그 창 안에는 날짜를 입력하여 그 날짜에 맞는 일일 박스 오피스 영화 데이터를 가지고 온다.
   - [X] 데이터들은 ListView에 Item마다 등록 진행 완료
   - [X] 다이얼로그에서 영화 로딩 버튼 클릭 시 영화 조회
 
- ## 2023-08-26
   - [X] 파이어베이스에 이미 존재하는 계정이면 Snackbar 알람 기능 구현
   - [X] ItemView Item 클릭 시 이전 다이얼로그로 돌아가서 선택한 영화 조회 
